...
def bd():
    ...
    try:
        ...
        if my_future[0]['long_qty']=='0':
            mail_text=my_future[0]['short_avg_cost']+'平多套保。上次动作：'+my_future[0]['long_avg_cost']+'开多'
        else:
            mail_text=my_future[0]['long_avg_cost']+'平空开多。上次动作：'+my_future[0]['short_avg_cost']+'开空'
